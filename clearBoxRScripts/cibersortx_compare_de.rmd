...
# lets take a look at IGFBP7 and CD44
...
melt_lcpm <- reshape2::melt(subset(lcpm, rownames(lcpm) %in% "ENSG00000026508")) %>% 
  as.data.frame() %>% 
  set_colnames(c("ensbl", "samplename", "log2cpm")) %>% 
  dplyr::left_join(., dge_list_obj$samples[, c("samplename", "Outcome", "Sex")], by = "samplename") %>%
  dplyr::left_join(., cibersortx_proportion[, c("samplename", "Maternal.CD8..Activated.T.Cells")], by = "samplename")

ggplot2::ggplot(data = melt_lcpm,
                aes(x = Maternal.CD8..Activated.T.Cells,
                    y = log2cpm)) +
  geom_point()
