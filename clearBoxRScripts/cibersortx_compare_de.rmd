...
## Correlation of cell-type proportions between samples

```{r}
# Assuming your data table is named 'data_table'
# Assuming 'samplename' column contains the sample names

# Extract cell names
cell_names <- c("Fetal.Mesenchymal.Stem.Cells", "Fetal.CD14..Monocytes")  # Add more cell names if needed

# Loop through each cell type
for (cell in cell_names) {
  # Subset data for the current cell type
  cell_data <- cibersortx_proportion[, c("samplename", cell)]
  
  # Pivot the data to have sample names as row names
  cell_data_pivot <- spread(cell_data, key = "samplename", value = cell)
  
  # Calculate correlation matrix
  correlation_matrix <- cor(cell_data_pivot)
  
  # Print correlation matrix for the current cell type
  cat("\nCorrelation matrix for", cell, ":\n")
...
# lcpm
lcpm <- edgeR::cpm(dge_list_obj, log = TRUE) %>%
  # drop the three sample not incuded 
...
for (i in 1:nrow(sig_de_overlap)) {
  ...
  cibersortx_proportion_rownames <- cibersortx_proportion %>%
    tibble::column_to_rownames("samplename")
  ...
  lcpm_GOI <- reshape2::melt(subset(lcpm, rownames(lcpm) %in% sig_de_overlap_ensbl$ensembl_gene_id[i])) %>% 
    ...
    dplyr::select(., -ensbl) %>%
    dplyr::left_join(., cibersortx_proportion, by = "samplename") %>%
