...
melt_lcpm <- melt(subset(lcpm, rownames(lcpm) %in% GOI_PE_male_UP$ensbl[i])) %>% 
  as.data.frame() %>% 
  set_colnames(c("ensbl", "samplename", "log2cpm")) %>% 
...
melt_lcpm <- melt(subset(lcpm, rownames(lcpm) %in% GOI_PE_male_DOWN$ensbl[i])) %>% 
  as.data.frame() %>% 
  set_colnames(c("ensbl", "samplename", "log2cpm")) %>% 
  left_join(., dge_list_obj$samples[, c("samplename", "Outcome", "Sex")], by = "samplename")  

png(filename=file.path(outdir_deUp, paste(GOI_PE_male_DOWN$ensbl[i], GOI_PE_male_DOWN$gene[i], "png",sep = ".")),
    width=800,
    height=800) 
