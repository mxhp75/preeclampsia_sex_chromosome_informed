...
print(num_common_strings) # 12349

# subset each table into the subset of common genes by gene name
intersect_cibersort_adjusted_mixture <- subset(cibersort, GeneSymbol %in% common_strings) %>%
  melt() %>%
  dplyr::select(., GeneSymbol, samplename = variable, cibersort = value) # 753289 3

intersect_tpm <- subset(tpm, GeneSymbol %in% common_strings) %>%
  melt() %>%
