...
plots <- vector("list", length(unique(log2_counts_to_plot$samplename)))
  ...
  for (i in seq_along(plots)) {
    ...
    for (col_name in names(cibersortx_reduced_sigMatrix)[-1]) {
      ...
      subset_with_sigMatrix_value <- cibersortx_reduced_sigMatrix[cibersortx_reduced_sigMatrix[, col_name] > 0, c("GeneSymbol", col_name)] %>%
        dplyr::left_join(., subset, by = "GeneSymbol")
        
      # Find the maximum value in the dataset
      max_value <- max(max(subset$tpm), max(subset$cibersort))
      # Find the minimum value in the dataset
      min_value <- min(min(subset$tpm), min(subset$cibersort))
      ...
      plots[[i]] <- ggplot(data = subset,
        ...
        labs(x = "log2 tpm + 0.01",
             y = "Cibersortx Mixture Matrix",
             title = samplename) +
        theme_bw() +
