...
genome_gtf_infile <- file.path(projectDir, "cleanData/gencode.v29.annotation.gtf")
input_counts_female <- file.path(projectDir, "rawData/test_v2.0.3_female_readCounts.txt")
input_counts_male <- file.path(projectDir, "rawData/test_v2.0.3_male_readCounts.txt")
...
output_female_cibersort_file <- file.path(outdir, paste0(expt_name, "bulkCountMatrix_female.txt"))
output_male_cibersort_file <- file.path(outdir, paste0(expt_name, "bulkCountMatrix_male.txt"))
...
# I'll output this for use with CIBERSORTx
small_tpm <- tpm[, 1:4]
male_tpm <- tpm[, male$samplename]
female_tpm <- tpm[, female$samplename]

male_tpm %>%
  tibble::as_tibble(rownames = NA) %>%
  tibble::rownames_to_column(var = "gene") %>% 
  # remove the number after the period
  tidyr::separate(., col = gene, into = c("gene", "right")) %>% 
  dplyr::select(., -right) %>%
  # save TPM count file for CIBERSTORTx
  write.table(file = output_male_cibersort_file,
              quote = FALSE,
              sep = "\t",
              col.names = TRUE,
              row.names = FALSE)
...
female_tpm %>%
  ...
  dplyr::select(., -right) %>%
  # save TPM count file for CIBERSTORTx
  write.table(file = output_female_cibersort_file,
              quote = FALSE,
              sep = "\t",
              col.names = TRUE,
              row.names = FALSE)
