...
outdir <- file.path(projectDir, "deduplicated_clearBox/output/57_sample_mesenchymalCovariate")
...
cibersortx_post_dedup_proportions <- read.table(file = input_cibersortx_post_dedup_proportions_file,
  ...
  # drop the samples no longer being considered
  # SCP4060 and SCP4010 failed sequencing
  # SCP3492 EOPE
  # SCP0148 outlier in the proportion estimates
  subset(., samplename %notin% c("SCP4060", "SCP3492", "SCP4010", "SCP0148"))
...
## Add cell-type proportions to the metadata file
...
logit_transform <- function(p, epsilon) {
  log((p + epsilon) / (1 - p + epsilon))
}

rownames(cibersortx_post_dedup_proportions) <- NULL
# Apply the logit transformation to every column in the data frame
proportions_transformed <- cibersortx_post_dedup_proportions %>% 
  data.frame() %>%
  tibble::column_to_rownames("samplename") %>%
  mutate(across(everything(), ~ logit_transform(.x, epsilon))) %>%
  tibble::rownames_to_column("samplename")

metadata_proportions <- dplyr::left_join(metadata, proportions_transformed, by = "samplename")
...
counts <- counts[, metadata_proportions$samplename]
dim(counts)
...
dge_list_obj <- DGEList(counts = counts_hgnc_clean %>%
                        ...
                        samples = metadata_proportions,
                        group = metadata_proportions$group,
...
model_noCovariate <- '~0 + sex_outcome + Fetal.Mesenchymal.Stem.Cells'
...
colnames(design_sex_PE) <- c(sex_outcome_levels, "Fetal.Mesenchymal.Stem.Cells")
...
write_csv(allTable_female_PE,
          file=file.path("allTable_female.csv"))
...
write_csv(allTable_male_PE,
          file=file.path("allTable_male.csv"))
...
write_csv(topTable_male_pe,
          file=file.path("topTable_male_pe.csv"))
...
#        filename = file.path(outdir, "volcano_male_covariate.png"),
#        units = "in",
#        width = 12,
#        height = 12,
#        dpi = 300)
