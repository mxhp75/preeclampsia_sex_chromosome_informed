...
- First, just the cell-type proportions and the un-merged modules.  
...
## run the module of interest
modules_of_interest <- MEs
# modules_of_interest <- mergedMEs

# ensure the samplenames in traits and eigengenes is the same
table(rownames(modules_of_interest) == rownames(col_ann_cell_type)) # TRUE 21

# Define the significance threshold
p_threshold <- 0.05

# Calculate the Pearson's Correlation Coefficient between the module eigengenes and the trait values
moduleTraitCor <- cor(modules_of_interest, col_ann_cell_type, use = "p")
moduleTraitPvalue <- corPvalueStudent(moduleTraitCor, nSamples)

# Create a matrix of correlations and their associated p-values
# Only include significant correlations in the text matrix
textMatrix <- ifelse(moduleTraitPvalue < p_threshold, 
                     paste(signif(moduleTraitCor, 2), "\n(", signif(moduleTraitPvalue, 1), ")", sep = ""),
                     "")  # Non-significant correlations will be blank

# Ensure dimensions match between textMatrix and moduleTraitCor
...
# Plot the heatmap with only significant correlations shown
labeledHeatmap(Matrix = moduleTraitCor,
               xLabels = colnames(col_ann_cell_type),
...
- Second, add the metadata of interest.  
```{r}
# Combine the cell-type proportions and the sample metadata of interest
traitsAll <- dplyr::full_join(col_ann %>% tibble::rownames_to_column("samplename"),
                              col_ann_cell_type %>% tibble::rownames_to_column("samplename"),
                              by = "samplename") %>%
  dplyr::left_join(., combined_bloodPressure,
                   by = "samplename") %>%
  tibble::column_to_rownames("samplename") %>%
  dplyr::select(., map, everything()) %>%
  dplyr::mutate(sex_outcome = recode(sex_outcome, "M_Control" = 1, "M_PE" = 2))  # Convert factors (outcome) to numeric


# Calculate the Pearson's Correlation Coefficient
moduleTraitAllCor <- cor(modules_of_interest, traitsAll, use = "p")
moduleTraitAllPvalue <- corPvalueStudent(moduleTraitAllCor, nSamples)

# Create a matrix of correlations and their associated p-values
# Only include significant correlations in the text matrix
textMatrix <- ifelse(moduleTraitAllPvalue < p_threshold, 
                     paste(signif(moduleTraitAllCor, 2), "\n(", signif(moduleTraitAllPvalue, 1), ")", sep = ""),
                     "")  # Non-significant correlations will be blank

# Ensure dimensions match between textMatrix and moduleTraitAllCor
dim(textMatrix) == dim(moduleTraitAllCor)

# Save the heatmap as a PNG file
# png(file = output_module_traitsAll_cor_heatmap_file,
#     width = 12,
#     height = 10,
#     units = "in",
#     res = 150)
...
# Plot the heatmap with only significant correlations shown
labeledHeatmap(Matrix = moduleTraitAllCor,
               xLabels = colnames(traitsAll),
               yLabels = colnames(modules_of_interest),
               ySymbols = colnames(modules_of_interest),
               colorLabels = FALSE,
               colors = blueWhiteRed(50),
               ...
               setStdMargins = FALSE,
               cex.text = 0.5,
               main = paste("Module-Trait Relationships (un-merged)"))
...
kME <- signedKME(datExpr = datExpr,
  ...
  left_join(., allTable_male, by = "hgnc_symbol") %>%
  dplyr::select(., hgnc_symbol, ensembl, chromosome = seqnames,
                gene_type, logFC, adj.P.Val, kME_brown, kME_green,
                kME_black, kME_pink, kME_tan, kME_blue, kME_greenyellow, kME_grey)


# Sort by module in descending order and select the top 10 rows
top10_hgnc <- kME %>%
  dplyr::arrange(desc(kME_tan)) %>%  # Sort descending by kMEbrown
  dplyr::slice(1:10) %>%              # Select top 10 rows
  dplyr::pull(hgnc_symbol)            # Return hgnc_symbol column
