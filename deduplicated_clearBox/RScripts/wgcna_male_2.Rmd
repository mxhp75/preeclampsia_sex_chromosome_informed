...
max_sbp_SCOPE_STOP_file <- file.path(projectDir, "metaData_SCOPE_STOP_output/SCOPE_STOP_max_sbp_dbp.csv")
...
output_wgcna_merged_modules_colours_file <- file.path(outdir, "wgcna_merged_modules_colours.rds")
output_wgcna_datExpr_file <- file.path(outdir, "wgcna_datExpr.rds")
output_wgcna_topTen_hubGenes_file <- file.path(outdir, "hub_genes_by_module.csv")
...
max_sbp_SCOPE_STOP <- read_csv(max_sbp_SCOPE_STOP_file)
...
## Determining the soft-thresholding power for network construction
...
colour_map <- c(
  "salmon" = "#191A1A",   # black
  "cyan" = "#FCAE3F",     # orange
  "black" = "#0B5A9C",    # darkblue
  "brown" = "#25AAE2",    # lightblue
  "grey60" = "#BE3597",   # pink
  "tan" = "#12A89D",      # green
  "grey" = "grey",        # grey
  "red" = "#DB4149",      # red
  "purple" = "#652D90"    # purple
)
...
for (i in 1:length(merge$newMEs)) {
  module_hex <- substring(names(merge$newMEs)[i], 3)
  genes <- colnames(datExpr)
  inModule <- merge$colors == module_hex
  ...
  # Build readable nodeAttr
  nodeAttr <- merge$colors[inModule]
  names(nodeAttr) <- modGenes
  nodeAttr <- sapply(nodeAttr, function(x) {
    if (x %in% names(reverse_colour_map)) reverse_colour_map[[x]] else x
  })
  
  # Safe file name (ie without the "#")
  safeModuleName <- gsub("#", "hex_", module_hex)
  
  # Export
  cyt <- exportNetworkToCytoscape(modTOM,
                                  edgeFile = file.path(outdir, paste0("merge_CytoscapeInput-edges-", safeModuleName, ".txt")),
                                  nodeFile = file.path(outdir, paste0("merge_CytoscapeInput-nodes-", safeModuleName, ".txt")),
                                  weighted = TRUE,
                                  # threshold = -1,
                                  threshold = 0.075,
                                  nodeNames = modGenes,
...
# Save edge file
write.csv(all_edges,
          file = paste0(outdir, "/combined_edges_0.075.csv"),
          row.names = FALSE)
...
# Save node file
write.csv(all_nodes,
          file = paste0(outdir, "/combined_nodes_0.075.csv"),
          row.names = FALSE)
...
```{r}
# use the reverse colour map object to rename the columns
# Get current column names
old_names <- colnames(mergedMEs)

# Extract the hex code from each "ME#..." column name
module_hexes <- substring(old_names, 3)

# Use the map to look up readable names
new_labels <- ifelse(module_hexes %in% names(reverse_colour_map),
                     reverse_colour_map[module_hexes],
                     module_hexes)

# Reattach "ME" prefix
new_names <- paste0("ME_", new_labels)

# Rename the columns
colnames(mergedMEs) <- new_names
...
```{r}
# create the traitsAll object
# start by joining the cell type proportiond and the metadata of interest
traitsAll <- dplyr::left_join(col_ann_cell_type %>% tibble::rownames_to_column("samplename"), dplyr::select(metadata, samplename, GestationalAge, BMI, maternalAge, sex_outcome), by = "samplename") %>%
  # next add the mean arterial pressure (map) measurement
  dplyr::left_join(., combined_bloodPressure, by = "samplename") %>%
  # and finally add the max sbp measurement
  dplyr::left_join(., max_sbp_SCOPE_STOP, by = "samplename") %>%
  tibble::column_to_rownames("samplename") %>%
  dplyr::select(., map, max_sbp, max_dbp, GestationalAge, BMI, maternalAge, sex_outcome, everything())
# # set the new factor levels
sex_outcome_levels <- c("F_Control", "M_Control", "F_PE", "M_PE")
traitsAll$sex_outcome <- factor(traitsAll$sex_outcome, levels=sex_outcome_levels)
traitsAll$sex_outcome <- as.numeric(traitsAll$sex_outcome)
...
# # lets prepare the mergedMEs with colour module names withut "ME_" (and drop the grey module)

# # Get the current column names
old_names <- colnames(mergedMEs)

new_names <- substring(old_names, 4)

# make a new version of the mergedMEs
mergedMEs_colour <- mergedMEs

# Apply new names
colnames(mergedMEs_colour) <- new_names

# Example usage for all traits including saving the headmap:
...
all_trait_cors <- create_module_trait_correlation(
  module_type = "merged",  # or "unmerged"
  MEs = MEs,
  mergedMEs = dplyr::select(mergedMEs_colour, -grey),
  trait_data = dplyr::select(traitsAll, -map, -max_sbp, -max_dbp),
  main_title = "Module-Trait Relationships (Male)"
  )

dev.off()
```
## Final module trait correlation figure
...
# --- Choose module eigengenes data ---
modules_of_interest <- dplyr::select(mergedMEs_colour, -grey) # merged version, drop grey

# --- Make sure sample names match ---
stopifnot(all(rownames(modules_of_interest) == rownames(traitsAll)))

# --- Subset trait data if needed ---
trait_data_sub <- dplyr::select(traitsAll, -map, -max_sbp, -max_dbp)

# --- Calculate correlations and p-values ---
nSamples <- nrow(modules_of_interest)
moduleTraitCor <- cor(modules_of_interest, trait_data_sub, use = "p")
moduleTraitPvalue <- corPvalueStudent(moduleTraitCor, nSamples)

# --- Create annotation text for significant correlations ---
p_threshold <- 0.05
textMatrix <- ifelse(moduleTraitPvalue < p_threshold,
...
# --- Replace the cell types in the column names to remove periods ---

# Create a named vector of new labels so the final plot has pretty labels
new_labels <- c(
  "GestationalAge" = "Gestational Age",
  "BMI" = "Maternal BMI",
  "maternalAge" = "Maternal Age",
  "sex_outcome" = "Preeclampsia (Male)",
  "Cytotrophoblast" = "Fetal Cytotrophoblasts",
  "Fetal.Extravillous.Trophoblasts" = "Fetal Extravillous Trophoblasts",
  "Fetal.Mesenchymal.Stem.Cells" = "Fetal Mesenchymal Stem Cells",
  "Fetal.GZMK..Natural.Killer" = "Fetal GZMK Natural Killer Cells",
  "Fetal.Memory.CD4..T.Cells" = "Fetal Memory CD4 T Cells",
  "Fetal.Hofbauer.Cells" = "Fetal Hofbauer Cells",
  "Fetal.Plasmacytoid.Dendritic.Cells" = "Fetal Plasmacytoid Dendritic Cells",
  "Fetal.GZMB..Natural.Killer" = "Fetal GZMB Natural Killer Cells",
  "Fetal.Endothelial.Cells" = "Fetal Endothelial Cells",
  "Fetal.Syncytiotrophoblast" = "Fetal Syncytiotrophoblast",
  "Fetal.Fibroblasts" = "Fetal Fibroblasts",
  "Fetal.Nucleated.Red.Blood.Cells" = "Fetal Nucleated Red Blood Cells",
  "Maternal.FCGR3A..Monocytes" = "Maternal FCGR3A Monocytes",
  "Maternal.Plasma.Cells" = "Maternal Plasma Cells",
...
colnames(moduleTraitCor) <- new_labels[colnames(moduleTraitCor)]

# sanity check the dimensions
dim_moduleTraitCor <- dim(moduleTraitCor) # 8 17

# --- Define color mapping ---
col_fun <- colorRamp2(c(-1, 0, 1), c("blue", "white", "red"))

# --- Named palette for module colours ---
module_colors_named <- c(
  black = "#191A1A",
  green = "#12A89D",
  darkblue = "#0B5A9C",
  lightblue = "#25AAE2",
...
# --- Get module names (ensure no 'ME' prefix) ---
module_names <- gsub("^ME", "", rownames(moduleTraitCor))

# --- Create annotation factor ---
module_factor <- factor(module_names, levels = names(module_colors_named))

# --- Row annotation object without legend ---
row_ha <- rowAnnotation(
  ModuleColour = module_factor,
  col = list(ModuleColour = module_colors_named),
  show_annotation_name = FALSE,
  show_legend = FALSE
)

# --- Plot with ComplexHeatmap ---
module_trait_correlation_heatmap <- Heatmap(
  ...
  width = ncol(moduleTraitCor) * unit(10, "mm"), 
  height = nrow(moduleTraitCor) * unit(8, "mm"),
  
  border = FALSE
)

# Save the trait correlation heatmap
png(file = file.path(outdir, "module_trait_correlation_heatmap.png"),
    width = 10,
    height = 8,
    units = "in",
    res = 300)
print(module_trait_correlation_heatmap)
dev.off()
```
...
# Extract module names from the Module Eigengenes (MEs) object
modNames <- substring(names(mergedMEs), 4)
...
#  Use the map to look up readable names
new_moduleColors <- ifelse(moduleColors %in% names(reverse_colour_map),
                     reverse_colour_map[moduleColors],
                     moduleColors)

column <- match(module, modNames)
moduleGenes <- new_moduleColors==module
sizeGrWindow(7, 7);
par(mfrow = c(1,1));
verboseScatterplot(abs(geneModuleMembership[moduleGenes, column]),
                   abs(geneTraitSignificance[moduleGenes, 1]),
...
- To identify structurally central genes driving the network’s topology within a module.  
```{r}
# calculate kIM
kIM <- intramodularConnectivity(adjMat = adjacency,
                                colors = new_moduleColors)
...
kIM_with_modules <- data.frame(
  ...
  Module = new_moduleColors,     # Add module assignments
...
lightblue_genes <- get_hub_genes(kIM_with_modules, "lightblue")
orange_genes <- get_hub_genes(kIM_with_modules, "orange")
darkblue_genes <- get_hub_genes(kIM_with_modules, "darkblue")
black_genes <- get_hub_genes(kIM_with_modules, "black")
pink_genes <- get_hub_genes(kIM_with_modules, "pink")
red_genes <- get_hub_genes(kIM_with_modules, "red")
purple_genes <- get_hub_genes(kIM_with_modules, "purple")
green_genes <- get_hub_genes(kIM_with_modules, "green")
...
plot_wgcna_modules <- function(expression_matrix, gene_module_table, output_dir = "module_plots") {
  ...
  for (module in modules) {
    ...
    module_data <- expr_long %>%
      dplyr::left_join(., ordered_metadata %>% dplyr::select(., samplename, sex_outcome), by = c("Sample" = "samplename")) %>%
      # Filter the data for the current module 
    ...
    p <- ggplot(module_data, aes(x = Sample,
                                 ...
                                 group = Gene,
                                 color = "red")) +
      # First layer: annotation bar using sex_outcome
      geom_tile(data = module_data,
                aes(x = Sample, y = max(module_data$Expression) * 1.05, fill = sex_outcome),
                height = max(module_data$Expression) * 0.05, width = 0.9,
                colour = NA) +
      # First fill scale: for annotation
      scale_fill_manual(
        name = "Sex Outcome",
        values = c(
          "F_Control" = "palegreen3",
          "F_PE" = "#004000",
          "M_Control" = "plum3",
          "M_PE" = "#2A0052"
        )
      ) +
      # Tell ggplot we're done with the first fill scale
      ggnewscale::new_scale_fill() +
      # Add your original geom_line layer
      geom_line(alpha = 0.5) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
            legend.position = "none") +  # Remove legend
    ...
    # Save the plot to the output directory
    ggsave(filename = file.path(outdir, paste0("mergedModule_", module, "_linePlot_anno.png")),
           plot = p,
           width = 10,
           height = 6)
  }
...
# recode the module colour names
merged_gene_module_table$Module <- sapply(
  merged_gene_module_table$Module,
  function(x) if (x %in% names(reverse_colour_map)) reverse_colour_map[[x]] else x
)
...
```{r}
# Fix column names in mergedMEs - remove underscore to match WGCNA expectations
# Change "ME_green" to "MEgreen", etc.
copy_mergedMEs <- mergedMEs
colnames(copy_mergedMEs) <- gsub("ME_", "ME", colnames(copy_mergedMEs))
...
top_genes <- setNames(lapply(unique(new_moduleColors), function(module) {
  # Select genes in the current module
  genes_in_module <- rownames(kME)[new_moduleColors == module]
...
}), unique(new_moduleColors)) # Name each list element by the module color

# Check for overlaps
top_genes_overlap <- Reduce(intersect, top_genes)
print(top_genes_overlap)

# Generate a table with module names and their top 10 hub genes
hub_genes_table <- data.frame(
  Module = names(top_genes),
  Hub_Genes = sapply(top_genes, function(genes) {
...
# View the table
print(hub_genes_table)

# Create a more formatted table using kable (if you have knitr installed)
library(knitr)
library(kableExtra)

# Formatted table for publication
formatted_table <- hub_genes_table %>%
  kable(col.names = c("Module", "Top 10 Hub Genes"),
        caption = "Top 10 hub genes for each module based on kME values") %>%
...
# Save as CSV
write.csv(hub_genes_table, output_wgcna_topTen_hubGenes_file, row.names = FALSE)


# find the hub gene for each module
topHubGene <- chooseTopHubInEachModule(datExpr = datExpr,
                                        colorh = new_moduleColors,
                                        omitColors = "grey",
                                        power = 2,
                                        type = "signed") %>%
  as.data.frame()
