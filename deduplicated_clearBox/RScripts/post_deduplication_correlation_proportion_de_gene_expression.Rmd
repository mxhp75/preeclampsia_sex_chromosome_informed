...
# projectDir <- "D:/VM_Projects/preeclampsia_sex_chromosome_informed" # windows style files
...
```{r}
# difference on GA between all PE and Control (sex naive)
...
cell_types <- c("Fetal.Mesenchymal.Stem.Cells", "CD14..Monocytes", "CD8..Activated.T.Cells", "Naive.CD4..T.Cells",
                "Naive.CD8..T.Cells", "Natural.Killer.T.Cells", "B.Cells", "Fetal.GZMK..Natural.Killer", "Fetal.Memory.CD4..T.Cells", "Fetal.Hofbauer.Cells", "Fetal.Plasmacytoid.Dendritic.Cells", "Fetal.GZMB..Natural.Killer", "Fetal.Endothelial.Cells", "Fetal.Syncytiotrophoblast", "Fetal.Fibroblasts", "Cytotrophoblast", "Fetal.Nucleated.Red.Blood.Cells", "Maternal.FCGR3A..Monocytes", "Maternal.Plasma.Cells", "Fetal.Extravillous.Trophoblasts")
...
# Updated ggplot code
ggplot(final_results, aes(x = reorder(cell_type, desc(odds_ratio)), y = odds_ratio)) +
...
ggplot(final_results_F,
       aes(x = reorder(cell_type, desc(odds_ratio)), y = odds_ratio)) +
  geom_point(aes(color = direction), size = 3) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper, color = direction), width = 0.2) +
  coord_flip() +
  geom_hline(yintercept = 1, linetype = "dashed") +
...
```{r}
# Ensure all cell types are included in both datasets
...
# but include all cell types (placing missing ones at the end)
cell_type_order <- final_results %>%
  arrange(desc(odds_ratio)) %>%
  pull(cell_type) %>%
  unique()  # Ensure unique values

# Add missing cell types to the order, placing them at the end
cell_type_order <- c(cell_type_order, setdiff(all_cell_types, cell_type_order))

# Ensure all cell types are included in both datasets and align the order
final_results <- final_results %>%
  complete(cell_type = all_cell_types,
           fill = list(odds_ratio = NA,
                       ci_lower = NA,
                       ci_upper = NA,
...
common_x_limits <- range(c(final_results$ci_lower,
                           final_results$ci_upper,
                           final_results_F$ci_lower,
                           final_results_F$ci_upper),
                         na.rm = TRUE)
...
print(male_plot)
ggsave(paste0(outdir, "/cell_type_odds_ratios_PE_male.pdf"), width = 12, height = 10, dpi = 300)
print(female_plot)
ggsave(paste0(outdir, "/cell_type_odds_ratios_PE_female.pdf"), width = 12, height = 10, dpi = 300)
...
annotation_df <- stacked_barchart_data %>%
  select(samplename, sex_outcome) %>%
  distinct()

# reorder the cell types to put fetal, maternal, unknown, together
stacked_barchart_data$variable <- factor(stacked_barchart_data$variable, 
                                         levels = c(
                                           "Cytotrophoblast",
...
ggplot() +
  ...
  geom_tile(data = annotation_df,
            aes(x = samplename, y = 1.05, fill = sex_outcome),
  ...
  # Second layer: stacked bar chart
  geom_bar(data = stacked_barchart_data,
           aes(x = samplename, y = value, fill = variable),
           stat = "identity") +

  # Second fill scale: for cell types
  scale_fill_manual(
    name = "Cell Type",
  ...
  # Labels and theming
  labs(
    # title = "Stacked Bar Chart of Post Deduplication Cell Type Proportions",
    x = "Sample",
    y = "Cell Type Proportion"
  ) +
...
- There are 43 genes that are both DE and in the signature matrix.  
...
# png(file=file.path(outdir, "male_post_dedup_correlation_heatmap_all_DE.png"),
#     width=14,
#     height=8,
#     units = "in",
#     res = 150)
pheatmap(final_correlation_results_plot_male,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         treeheight_row = 0,   # This removes row dendrogram
         show_colnames = FALSE, # This removes column names
         annotation_col = annotation_DE,
         main = "Correlations between expression of DE genes and cell types - Male")
# dev.off()

# First create the male heatmap and store the clustering information
## drop the natural killer cells as zero in all samples
final_correlation_results_plot_male <- final_correlation_results_plot_male[rownames(final_correlation_results_plot_male) != "Natural.Killer.T.Cells", ]

male_heatmap <- pheatmap(final_correlation_results_plot_male,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         treeheight_row = 0,
         show_colnames = FALSE,
...
# Get the clustering order from the male heatmap
male_row_order <- male_heatmap$tree_row$order
male_col_order <- male_heatmap$tree_col$order
```
...
```{r}
# create a vector of female sample IDs
female_samplename <- metadata_quality %>%
  filter(Sex == "F") %>%
  pull(samplename)
...
final_correlation_results_plot_female <- final_correlation_results %>%
  mutate_all(~replace(., is.na(.), 0)) %>%
    column_to_rownames("Cell_Type")
## drop the natural killer cells as zero in all samples
final_correlation_results_plot_female <- final_correlation_results_plot_female[rownames(final_correlation_results_plot_female) != "Natural.Killer.T.Cells", ]
...
### Combined Male and Female Heatmaps

```{r}
# Create common parameters for both heatmaps to ensure consistency
common_height <- 20  # Adjust this value as needed
common_width <- 2   # Adjust this value as needed

# First create and plot the male heatmap to get the clustering order
male_heatmap <- pheatmap(final_correlation_results_plot_male,
                         cluster_rows = TRUE,
                         cluster_cols = TRUE,
                         treeheight_row = 0,
                         ...
                         annotation_col = annotation_DE,
                         main = "Male",
                         cellheight = common_height,
                         cellwidth = common_width,
                         silent = TRUE) # add or subtract the heatmap colour legend

# Get the ordered row and column names from the male heatmap
ordered_rows <- rownames(final_correlation_results_plot_male)[male_heatmap$tree_row$order]
ordered_cols <- colnames(final_correlation_results_plot_male)[male_heatmap$tree_col$order]

# Reorder the female matrix to match male ordering
final_correlation_results_plot_female_ordered <- final_correlation_results_plot_female[ordered_rows, ordered_cols]

# Create annotation dataframe with the correct ordering
annotation_DE_ordered <- data.frame(
  DE_direction = annotation_DE[ordered_cols, "DE_direction"],
  row.names = ordered_cols
)

# Create female heatmap with matched ordering
female_heatmap <- pheatmap(final_correlation_results_plot_female_ordered,
                           cluster_rows = FALSE,
                           cluster_cols = FALSE,
                           treeheight_row = 0,
                           show_colnames = FALSE,
                           annotation_col = annotation_DE_ordered,
                           main = "Female",
                           cellheight = common_height,
                           cellwidth = common_width,
                           silent = FALSE) # add or subtract the heatmap colour legend


# Align heatmaps at the bottom
male_grob <- male_heatmap$gtable
female_grob <- female_heatmap$gtable

# Normalize the heights for alignment
max_height <- grid::unit.pmax(male_grob$heights, female_grob$heights)
male_grob$heights <- max_height
female_grob$heights <- max_height

# Combine the plots
heatmap_combined <- grid.arrange(
  male_grob, female_grob,
  ncol = 2,
  top = textGrob("Correlations between expression of DE genes and cell types", 
...
ggsave(paste0(outdir, "/20250519_combined_heatmaps.png"), heatmap_combined, width = 18, height = 8, dpi = 300)
